...
export class MockDataService {
  ...
  constructor(
    ...
    this.mockUser = new User('100', 'Real Name', 'NickName','mail@gmail.com', linkUser, 0, 'User');

    let linkActiveUser: string = CONSTANTS.MOCK.USER_IMAGE;
    this.mockActiveUser = new User('200','Vitalik Priadka', 'SiriusDark', 'vitalik@gmail.com', linkActiveUser, 0, 'Admin');

    let commentDate = dateTimeService.unixToDate('1541538783');
    this.mockComment = new Comment('300', this.mockUser, 'Обожаю ваш ресурс! Самые авторитетные и неподкупные новости, всегда акктуально свежо, молодо и вообще всем селом читаем! NodeJS + TypeScript (JavaScript) впервые дали возможность быть FullStack и писать только на одном ЯП. Тем самым упрощая процесс профессионального роста.',commentDate, 100);

    this.categorysList = [
      new Category('111', 'Anime', 0),
      new Category('112', 'Web', 10),
      new Category('113', 'Design', 20,),
      new Category('114', 'Android', 12,),
      new Category('115', 'Toasters', 14,),
      new Category('116', 'iOS', 7,),
      new Category('117', 'Space', 15,),
      new Category('118', 'Navalny', 6,),
    ];
    ...
    let title: string = 'Фулстеки — это вечные мидлы. Не идите по этому пути, если не хотите страдать. Title us test',
      ...
      categories = [this.categorysList[0], this.categorysList[3], this.categorysList[6]];
    this.mockNews = new News ( '100', this.mockUser, date, title, text, linkNews, categories,100, 5);
  }
